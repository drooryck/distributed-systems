...
class TestInvalidLogin(BaseTest):
    def test_invalid_login(self):
        ...
        response = self.receive_response()
        self.assertEqual(response["status"], "error", "❌ Test Failed: Logged in without a password!")
        ...
        response = self.receive_response()
        self.assertEqual(response["status"], "error", "❌ Test Failed: Logged in without a username!")

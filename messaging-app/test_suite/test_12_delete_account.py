...
class TestDeleteAccount(BaseTest):
    def test_cannot_delete_without_login(self):
        ...
        self.assertEqual(response["data"]["status"], "error", "❌ Should fail if no user is logged in.")
        self.assertIn("not currently logged in", response["data"]["msg"], "❌ Error message should indicate no login.")
    ...
    def test_delete_account_removes_user_and_messages(self):
        ...
        self.assertEqual(login_resp["data"]["status"], "ok", "❌ Alice should log in successfully.")

        # Alice sends message to Bob
        self.send_message("send_message", {"sender": "Alice", "recipient": "Bob", "content": "Hello Bob!"})
        msg_response = self.receive_response()
        self.assertEqual(msg_response["data"]["status"], "ok", "❌ Should store the message from Alice to Bob.")

        # Delete Alice's account
        self.send_message("delete_account", {})
        del_response = self.receive_response()
        print(del_response)
        self.assertEqual(del_response["data"]["status"], "ok", "❌ Deleting account should succeed.")
        self.assertIn("has been deleted", del_response["data"]["msg"], "❌ Should confirm account deletion message.")

        # Attempt to log in again as Alice -> user should not exist
        self.send_message("login", {"username": "Alice", "password": "secret"})
        re_login_response = self.receive_response()
        self.assertEqual(re_login_response["data"]["status"], "error", "❌ Should fail - user no longer exists.")
        self.assertIn("not found", re_login_response["data"]["msg"], "❌ Should indicate 'Username not found.'")

        # Attempt to log in as Bob
        self.send_message("login", {"username": "Bob", "password": "bobpass"})
        bob_login_resp = self.receive_response()
        self.assertEqual(bob_login_resp["data"]["status"], "ok", "❌ Bob should still exist and be able to log in.")

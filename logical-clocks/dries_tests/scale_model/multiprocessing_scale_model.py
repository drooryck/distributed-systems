...
class VirtualMachine(logical_clock_pb2_grpc.VirtualMachineServicer):
    ...
    def random_event(self):
        """
        With probability:
         - 3/10 => SEND to a subset of peers (1, 2, or both).
         - 7/10 => INTERNAL event.
        """
...
def single_run(duration, run_index, output_dir):
    ...
    Spawns 3 VM processes, sleeps, stops them via RPC, merges their logs,
    then calls analyze_logs to produce plots.
    """
    # Create the folder for this run (e.g., 'results/10seconds/run1')
    os.makedirs(output_dir, exist_ok=True)
    
    # Random clock rates for the 3 VMs
    clock_rates = [random.randint(1, 6) for _ in range(3)]
    
    # Hard-code ports for each VM
    ports = [50051, 50052, 50053]
    
    # Peers: for VM0, peers = [("localhost", 50052), ("localhost", 50053)], etc.
    peer_lists = [
    ...
    log_files = [
        os.path.join(output_dir, f"vm0_events.csv"),
        os.path.join(output_dir, f"vm1_events.csv"),
        os.path.join(output_dir, f"vm2_events.csv"),
    ]

...
// Server ID within the cluster, from the command line: node src/server.js <id>
const SERVER_ID = parseInt(process.argv[2] || '0', 10);

// Initialize cluster manager (falls back to standalone mode on failure)
...
try {
  ...
  console.error('Failed to initialize cluster manager, running standalone:', error.message);
}

// Client-facing port comes from cluster-config.json for this server ID
const PORT = process.env.NODE_ENV === 'test'
  ? 0
  : (process.env.PORT || (clusterManager ? clusterManager.serverConfig.port : 3001));
...
// Leader redirection middleware: page loads on a follower are redirected to the leader
...
function updateRoomGameState(roomCode) {
  ...
  // Leader replicates the frame to followers so a failover loses at most a frame
  if (clusterManager && clusterManager.isLeaderServer()) {
    clusterManager.broadcastState({
      action: 'gameStateUpdate',
      roomCode,
      gameState: activePlayersToArray(gameState),
      timestamp: Date.now()
    });
  }
}
...
io.on('connection', (socket) => {
  ...
  // Let the client discover which server is the leader
  ...
  socket.on('rejoinRoom', (data) => {
    ...
    if (!rooms[roomCode]) {
      // A follower may not have the room yet; ask the leader for a snapshot
      if (clusterManager && !clusterManager.isLeaderServer()) {
        clusterManager.requestRoomFromLeader(roomCode)
          .then(result => {
            if (result && result.exists && result.room && result.room.gameState) {
              const recovered = result.room;
              recovered.gameState.activePlayers = getActivePlayersAsSet(recovered.gameState);
              rooms[roomCode] = {
                gameState: recovered.gameState,
          ...
          .catch(error => {
            debugLog('events', `Error requesting room from leader: ${error.message}`);
            socket.emit('error', { message: 'Failed to recover room data' });
          });
        return; // Async recovery in flight
      }
...
// Restart the authoritative game loop for a room after becoming leader
function restartGameLoopAsLeader(roomCode) {
  ...
  if (rooms[roomCode].gameState.appPhase === 'playing') {
    ...
    // Lobby/gameover rooms just need their state broadcast to local clients
    roomGameLoops[roomCode] = setInterval(() => {
      if (rooms[roomCode] && rooms[roomCode].gameState.activePlayers.size > 0) {
        io.to(roomCode).emit('gameState', rooms[roomCode].gameState);
      }
...
// Periodic full-state replication interval; guarded so repeated
// leadership changes never stack multiple intervals.
let stateReplicationInterval = null;

function setupCluster() {
  clusterManager.initialize();

  // Leader answers room snapshot requests from followers
  clusterManager.onRoomRequest = (roomCode) => {
    const room = roomCode ? rooms[roomCode.toUpperCase()] : null;
    if (!room) return null;
    return {
      gameState: activePlayersToArray(room.gameState),
  ...
  clusterManager.on('became-leader', () => {
    ...
    if (stateReplicationInterval) {
      clearInterval(stateReplicationInterval);
    }
    stateReplicationInterval = setInterval(() => {
      if (clusterManager.isLeaderServer()) {
        clusterManager.broadcastFullGameState(rooms);
      } else {
        clearInterval(stateReplicationInterval);
        stateReplicationInterval = null;
      }
    }, 3000);

    // Take over game loops for every replicated room, not just playing ones
    Object.keys(rooms).forEach(roomCode => restartGameLoopAsLeader(roomCode));
  });
  ...
  clusterManager.on('stepped-down', () => {
    ...
    // Followers must not run loops; they relay state as it arrives from the leader
    Object.keys(roomGameLoops).forEach(roomCode => {
      clearInterval(roomGameLoops[roomCode]);
      delete roomGameLoops[roomCode];
    });
  });

  // Apply state replicated from the leader
  clusterManager.on('state-update', (data) => {
    if (clusterManager.isLeaderServer()) return;

    if (data.action === 'fullStateSync' && data.rooms) {
      Object.keys(data.rooms).forEach(roomCode => {
        ...
        if (!rooms[roomCode]) {
          ...
          // Replicated rooms get a cleanup timer too, so followers don't leak rooms
          setTimeout(() => checkRoomActivity(roomCode), inactiveRoomCleanupTime);
          debugLog('cluster', `Added room ${roomCode} from full state sync`);
        } else {
          rooms[roomCode].gameState = roomData.gameState;
          rooms[roomCode].lastActivity = roomData.lastActivity || Date.now();
        }
      });
    }
      ...
      if (!rooms[data.roomCode]) {
        ...
        setTimeout(() => checkRoomActivity(data.roomCode), inactiveRoomCleanupTime);
        debugLog('cluster', `Created room ${data.roomCode} from real-time update`);
      } else {
        rooms[data.roomCode].gameState = data.gameState;
        rooms[data.roomCode].lastActivity = Date.now();
      }

      // Relay the fresh state to any clients connected to this follower
      const roomSocketIds = io.sockets.adapter.rooms.get(data.roomCode);
      if (roomSocketIds && roomSocketIds.size > 0) {
        io.to(data.roomCode).emit('gameState', rooms[data.roomCode].gameState);
      }
    }
  });
}
...
app.get('/status', (req, res) => {
  res.json({
    serverId: SERVER_ID,
    port: PORT,
    isLeader: clusterManager ? clusterManager.isLeaderServer() : true,
    leaderId: clusterManager ? clusterManager.getLeaderId() : SERVER_ID,
...
// Gracefully kill this server (used by fault-injection tests and failover demos)
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Tetris server ${SERVER_ID} running on port ${server.address().port}`);
  if (clusterManager) {
    setupCluster();
  }
});

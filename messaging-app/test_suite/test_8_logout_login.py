...
class TestLogoutLogin(BaseTest):
    def test_logout_and_login(self):
        ...
        self.assertEqual(response["data"]["status"], "ok", "❌ Should log in successfully")

        # Logout
        self.send_message("logout", {})
        response = self.receive_response()
        self.assertEqual(response["data"]["status"], "ok", "❌ Logout should succeed")

        # Attempt action while logged out
        self.send_message("send_message", {"sender": "Alice", "recipient": "Bob", "content": "Test message"})
        response = self.receive_response()
        self.assertEqual(response["data"]["status"], "error", "❌ Should not allow sending while logged out")

        # Login again
        self.send_message("login", {"username": "Alice", "password": "secret"})
        response = self.receive_response()
        self.assertEqual(response["data"]["status"], "ok", "❌ Should log in again after logout")

...
class TestDeleteMultipleMessages(BaseTest):
    def test_delete_multiple_messages(self):
        ...
        for content in messages_to_send:
            self.send_message("send_message", {"sender": "Alice", "recipient": "Bob", "content": content}, is_response=0)
            self.receive_response()
        ...
        self.send_message("fetch_away_msgs", {"num_messages": 10}, is_response=0)
        fetch_response = self.receive_response()
        delivered_list = fetch_response["msg"]
        ...
        self.assertEqual(len(delivered_response["msg"]), 3, "❌ Still expecting 3 delivered messages")

        # Delete 2 of them
        msg_ids = [m["id"] for m in delivered_list[:2]]  # first 2 messages
        self.send_message("delete_messages", {"message_ids_to_delete": msg_ids}, is_response=0)
        delete_resp = self.receive_response()
        self.assertEqual(delete_resp["deleted_count"], 2, "❌ Should delete exactly 2 messages")

        # Now only 1 delivered message should remain
        self.send_message("send_messages_to_client", {}, is_response=0)
        final_delivered = self.receive_response()
        self.assertEqual(len(final_delivered["msg"]), 1, "❌ Only 1 message should remain after deleting 2")

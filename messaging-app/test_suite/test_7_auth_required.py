...
class TestAuthenticationRequired(BaseTest):
    def test_fetch_messages_without_login(self):
        ...
        self.assertEqual(response["data"]["status"], "error", "❌ Test Failed: Fetching messages without login should not be allowed!")
    ...
    def test_send_message_without_login(self):
        ...
        response = self.receive_response()
        self.assertEqual(response["data"]["status"], "error", "❌ Test Failed: Sending messages without login should not be allowed!")
    ...
    def test_user_cannot_send_as_another_user(self):
        ...
        self.send_message("send_message", {"sender": "Bob", "recipient": "Charlie", "content": "Message from Alice as Bob!"})
        response = self.receive_response()
        self.assertEqual(response["data"]["status"], "error", "❌ Test Failed: Alice should not be able to send messages as Bob!")
